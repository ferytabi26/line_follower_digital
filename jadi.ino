...
void setup() {
  ...
  for(int i=0;i<50;i++){
    tim[i] = EEPROM.read(20+i);
    sen[i] = EEPROM.read(70+i);
    mot[i] = EEPROM.read(120+i);
    del[i] = EEPROM.read(170+i);
    pau[i] = EEPROM.read(220+i);
  }
  lcd.init();
  lcd.backlight();
  lcd.setCursor(4,0);
...
//      }
    ...
    case 4 :
      ...
      if(atas){lcd.clear();state++;delay(500);}
      if(bawah){lcd.clear();state--;delay(500);}
      if(ok){lcd.clear();state=41;delay(300);}
    break;
    ////////////////////////////////////////mission///////////////////////////////////////////////////
    case 41:
      lcd.setCursor(0,0);lcd.print("misi ke = ");lcd.print(misi);
      if(atas){lcd.clear();misi++;delay(300);}
      if(bawah){lcd.clear();misi--;delay(300);}
      if(cancel){lcd.clear();state=4;delay(300);}
      if(ok){lcd.clear();state=411;delay(300);}
    break;
    case 411:
      lcd.setCursor(0,0);lcd.print("T=");lcd.print(tim[misi]);lcd.print("<");
      lcd.setCursor(0,1);lcd.print("S=");lcd.print(sen[misi]);
      lcd.setCursor(6,0);lcd.print("M=");lcd.print(mot[misi]);
      lcd.setCursor(6,1);lcd.print("D=");lcd.print(del[misi]);
      lcd.setCursor(13,0);lcd.print("P=");lcd.print(pau[misi]);
      lcd.setCursor(13,1);lcd.print("ok");
      if(ok){lcd.clear();tim[misi]++;delay(300);}
      if(cancel){lcd.clear();tim[misi]--;delay(300);}
      if(atas){lcd.clear();state=411;delay(300);}
      if(bawah){lcd.clear();state=412;delay(300);}
    break;
    case 412:
      lcd.setCursor(0,0);lcd.print("T=");lcd.print(tim[misi]);
      lcd.setCursor(0,1);lcd.print("S=");lcd.print(sen[misi]);lcd.print("<");
      lcd.setCursor(6,0);lcd.print("M=");lcd.print(mot[misi]);
      lcd.setCursor(6,1);lcd.print("D=");lcd.print(del[misi]);
      lcd.setCursor(13,0);lcd.print("P=");lcd.print(pau[misi]);
      lcd.setCursor(13,1);lcd.print("ok");
      if(ok){lcd.clear();sen[misi]++;delay(300);}
      if(cancel){lcd.clear();sen[misi]--;delay(300);}
      if(atas){lcd.clear();state=411;delay(300);}
      if(bawah){lcd.clear();state=413;delay(300);}
    break;
    case 413:
      lcd.setCursor(0,0);lcd.print("T=");lcd.print(tim[misi]);
      lcd.setCursor(0,1);lcd.print("S=");lcd.print(sen[misi]);
      lcd.setCursor(6,0);lcd.print("M=");lcd.print(mot[misi]);lcd.print("<");
      lcd.setCursor(6,1);lcd.print("D=");lcd.print(del[misi]);
      lcd.setCursor(13,0);lcd.print("P=");lcd.print(pau[misi]);
      lcd.setCursor(13,1);lcd.print("ok");
      if(ok){lcd.clear();mot[misi]++;delay(300);}
      if(cancel){lcd.clear();mot[misi]--;delay(300);}
      if(atas){lcd.clear();state=412;delay(300);}
      if(bawah){lcd.clear();state=414;delay(300);}
    break;
    case 414:
      lcd.setCursor(0,0);lcd.print("T=");lcd.print(tim[misi]);
      lcd.setCursor(0,1);lcd.print("S=");lcd.print(sen[misi]);
      lcd.setCursor(6,0);lcd.print("M=");lcd.print(mot[misi]);
      lcd.setCursor(6,1);lcd.print("D=");lcd.print(del[misi]);lcd.print("<");
      lcd.setCursor(13,0);lcd.print("P=");lcd.print(pau[misi]);
      lcd.setCursor(13,1);lcd.print("ok");
      if(ok){lcd.clear();del[misi]++;delay(300);}
      if(cancel){lcd.clear();del[misi]--;delay(300);}
      if(atas){lcd.clear();state=413;delay(300);}
      if(bawah){lcd.clear();state=415;delay(300);}
    break;
    case 415:
      lcd.setCursor(0,0);lcd.print("T=");lcd.print(tim[misi]);
      lcd.setCursor(0,1);lcd.print("S=");lcd.print(sen[misi]);
      lcd.setCursor(6,0);lcd.print("M=");lcd.print(mot[misi]);
      lcd.setCursor(6,1);lcd.print("D=");lcd.print(del[misi]);
      lcd.setCursor(13,0);lcd.print("P=");lcd.print(pau[misi]);lcd.print("<");
      lcd.setCursor(13,1);lcd.print("ok");
      if(ok){lcd.clear();pau[misi]++;delay(300);}
      if(cancel){lcd.clear();pau[misi]--;delay(300);}
      if(atas){lcd.clear();state=414;delay(300);}
      if(bawah){lcd.clear();state=416;delay(300);}
    break;
    case 416:
      lcd.setCursor(0,0);lcd.print("T=");lcd.print(tim[misi]);
      lcd.setCursor(0,1);lcd.print("S=");lcd.print(sen[misi]);
      ...
      lcd.setCursor(6,1);lcd.print("D=");lcd.print(del[misi]);
      lcd.setCursor(13,0);lcd.print("P=");lcd.print(pau[misi]);
      lcd.setCursor(13,1);lcd.print("ok");lcd.print("<");
      if(ok){lcd.clear();state=4151;delay(300);}
      if(cancel){lcd.clear();state=41;delay(300);}
      if(atas){lcd.clear();state=414;delay(300);}
      if(bawah){lcd.clear();state=415;delay(300);}
    break;
    case 4151 :
      lcd.clear();
      EEPROM.update((20+misi),tim[misi]);
      EEPROM.update((70+misi),sen[misi]);
      EEPROM.update((120+misi),mot[misi]);  
      EEPROM.update((170+misi),del[misi]);
      EEPROM.update((220+misi),pau[misi]);
      delayMicroseconds(100);
      lcd.setCursor(0,0);lcd.print("tersimpan");delay(500);lcd.clear();state=41;
    break;
    /////////////////////////////////////////////////////////////////////////////////////////////////
    case 5 :
...
//////////////////////////////////////////////start/////////////////////////////////////////////////
      if(ok){
        ...
        while(keadaan){
          ...
          simpan = 0;
          timing = waktuSekarang-now;
          if (timing > (tim[counter]*100)){
            if(senValue[sen[counter]]==1){
              delay(pau[counter]*100);
              motionku(mot[counter]);
              delay(del[counter]*100);
              now = waktuSekarang;
              timing = waktuSekarang-now; 
              counter++;
            }
...
//          delay(10);
    ...
    case 6 :
      ...
      if(atas){lcd.clear();state++;delay(500);}
      if(bawah){lcd.clear();state--;delay(500);}
      if(ok){lcd.clear();state=61;delay(300);}
      break;
    case 61 :
      for (int i = 0 ; i < EEPROM.length() ; i++) {
         EEPROM.write(i, 0);
      }
      lcd.setCursor(0,0);lcd.print("Sudah Bersih..");delay(1000);lcd.clear();state=6;
      break;
    //submenu setmode
    case 11 :
...
void motionku(byte motion){
  if (motion==1){
    analogWrite(en_ka,180);
    digitalWrite(in1_ka,HIGH);
    digitalWrite(in2_ka,LOW);
    analogWrite(en_ki,180);
    digitalWrite(in1_ki,HIGH);
    digitalWrite(in2_ki,LOW); 
  }
  if (motion==2){
